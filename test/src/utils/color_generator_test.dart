...
void main() {
  group('ColorGenerator', () {
    test('nextColor returns a fully opaque Color', () {
      ...
      expect((color.toARGB32() >> 24) & 0xFF, equals(0xFF));
    });

    test('nextColor RGB channels are within 0–255', () {
      final Color color = ColorGenerator.generateRandomColor();
      expect((color.toARGB32() >> 16) & 0xFF, inInclusiveRange(0, 255));
      expect((color.toARGB32() >> 8) & 0xFF, inInclusiveRange(0, 255));
      expect(color.toARGB32() & 0xFF, inInclusiveRange(0, 255));
    });
